...
Shiny.addCustomMessageHandler("setHelpContent",
  ...
  function(message){
    ...
    // IF a nextPage is provided the done button is replaced with a next page
    // button
    if(message.nextPage !== null){
      intro.setOption('doneLabel', 'Next page').oncomplete(function() {
        // When we click next page the appropriate tab is selected with Jquery
        // and its content will be loaded
        $('[data-value=' + message.nextPage + ']').tab('show');
        // This is to let shiny know we went to the next page while displaying
        // the help and the help should be displayed when opening this page.
        // The help cannot be opend here directly because this creates an
        // infinite loop which results in a stack overflow even with a timeout.
...
Shiny.addCustomMessageHandler("startHelp",
  ...
  function(message) {
    ...
    intro.onchange(function(el){
      // commented bc I dont want it to read the message
...
function startSpeech(){
  ...
  $.each(arr, function () {
    ...
    u.lang='en-US'; // for US english, en-GR for british
